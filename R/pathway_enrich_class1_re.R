...
response <- POST(
  ...
  body = toJSON(list(Symbols = DEGs), auto_unbox = TRUE),
  add_headers(`Content-Type` = "text/json"),
  config = config(ssl_verifypeer = FALSE)
)

document <- fromJSON(content(response, as = "text", encoding = "UTF-8"))
...
getEnrich <- POST(
  ...
  body = toJSON(list(Genes = gene_map$Entrez), auto_unbox = TRUE),
  add_headers(`Content-Type` = "text/json"),
  config = config(ssl_verifypeer = FALSE)
)
signatures <- fromJSON(content(getEnrich, as = "text", encoding = "UTF-8"))
...
save(finalMatrix, path_enrich, file = "DB_pathway_class1.RData")
