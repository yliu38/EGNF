...
for spl_id in label_train['sid']:
    ...
    if zero_rows.any():
        continue
    edge_attr = torch.tensor(edges.loc[edges['sample_id'] == spl_id]['num_samples'].to_numpy(), dtype=torch.float32)
...
with open('../ten_times/pickles/GCN_DB_'+ str(job_index) +'.pickle' , 'wb') as fl:
