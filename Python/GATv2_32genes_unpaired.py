...
class GATv2Classifier(torch.nn.Module):
    def __init__(self, in_channels, hidden_dim, num_classes, num_layers, heads=1, dropout=0.5, edge_attr_dim=3):
        ...
        self.convs.append(GATv2Conv(in_channels, hidden_dim, heads=heads, concat=True, edge_dim=self.edge_attr_dim))
        for _ in range(num_layers - 2):
            self.convs.append(GATv2Conv(hidden_dim * heads, hidden_dim, heads=heads, concat=True, edge_dim=self.edge_attr_dim))
        self.convs.append(GATv2Conv(hidden_dim * heads, num_classes, heads=1, concat=False, edge_dim=self.edge_attr_dim))
...
class GATWrapper(BaseEstimator):
    def __init__(self, hidden_dim=16, num_layers=2, learning_rate=0.01, batch_size=8, epochs=100, dropout=0.5, weight_decay=0.0001, edge_attr_dim=3, heads=1):
    ...
    def fit(self, X, y=None):
        ...
        for fold, (train_idx, val_idx) in enumerate(kf.split(dataset, [data.y.item() for data in dataset])):
            ...
            model = GATv2Classifier(  # Assuming you now use 
